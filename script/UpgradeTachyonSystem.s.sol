...
contract UpgradeTachyonSystem is Script {
    ...
    function updateDeploymentsFile(ExistingContracts memory existing, NewImplementations memory newImpls) internal {
        string memory json = string.concat(
            ...
            '  "contracts": {\n',
            '    "TachyonToken": {\n',
            '      "proxy": "', vm.toString(existing.tachyonTokenProxy), '",\n',
            '      "implementation": "', vm.toString(newImpls.tachyonTokenImpl), '",\n',
            '      "upgraded": true\n',
            '    },\n',
            '    "GreenVerifier": {\n',
            '      "proxy": "', vm.toString(existing.greenVerifierProxy), '",\n',
            '      "implementation": "', vm.toString(newImpls.greenVerifierImpl), '"\n',
            '    },\n',
            '    "AIOracle": {\n',
            '      "proxy": "', vm.toString(existing.aiOracleProxy), '",\n',
            '      "implementation": "', vm.toString(newImpls.aiOracleImpl), '"\n',
            '    },\n',
            '    "NodeRegistry": {\n',
            '      "proxy": "', vm.toString(existing.nodeRegistryProxy), '",\n',
            '      "implementation": "', vm.toString(newImpls.nodeRegistryImpl), '"\n',
            '    },\n',
            '    "RewardManager": {\n',
            '      "proxy": "', vm.toString(existing.rewardManagerProxy), '",\n',
            '      "implementation": "', vm.toString(newImpls.rewardManagerImpl), '"\n',
            '    },\n',
            '    "JobManager": {\n',
            '      "proxy": "', vm.toString(existing.jobManagerProxy), '",\n',
            '      "implementation": "', vm.toString(newImpls.jobManagerImpl), '"\n',
            '    }\n',
            '  }\n',
            '}'
        );
