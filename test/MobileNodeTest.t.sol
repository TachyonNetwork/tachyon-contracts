...
contract MobileNodeTest is Test {
    ...
    function testDeviceProfilesInitialized() public {
        ...
        // Test server profile
        (minStake, maxTasks,,,,,,,,,) = nodeRegistry.deviceProfiles(NodeRegistry.NodeDeviceType.SERVER_RACK_2U);
    ...
    function testDeviceTypeQueries() public {
        ...
        address[] memory smartphones = nodeRegistry.getNodesByDeviceType(NodeRegistry.NodeDeviceType.SMARTPHONE);
        assertEq(smartphones.length, 1, "Should have 1 smartphone");
        assertEq(smartphones[0], mobileUser, "Should be the mobile user");

        address[] memory servers = nodeRegistry.getNodesByDeviceType(NodeRegistry.NodeDeviceType.SERVER_TOWER);
