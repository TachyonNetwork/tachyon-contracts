    ...
    UUPSUpgradeable 
{
    ...
    function updateCapabilities(
        NodeCapabilities calldata newCapabilities,
        bytes calldata attestationData
    ...
    function updateActivity(address nodeAddress) external {
        ...
        require(node.registered, "Not registered");
        require(
            msg.sender == nodeAddress || hasRole(DEFAULT_ADMIN_ROLE, msg.sender),
            "Unauthorized"
        );
    ...
    function getNodesForTask(
        uint256 minCPU,
        uint256 minRAM,
        bool requireGPU,
        bool preferGreen
    ) external view returns (address[] memory suitableNodes) {
        uint256 count = 0;
        ...
        for (uint256 i = 0; i < activeNodes.length; i++) {
            ...
            // Check requirements
            if (node.capabilities.cpuCores >= minCPU &&
                node.capabilities.ramGB >= minRAM &&
                (!requireGPU || node.capabilities.hasGPU) &&
                node.reputation >= 30 &&
    ...
    function isNodeEligible(address nodeAddress) external view returns (bool) {
        ...
        return node.registered &&
               !slashedNodes[nodeAddress] &&
               node.stake >= requiredMinStake &&
               block.timestamp <= node.attestation.validUntil &&
               block.timestamp - node.lastActiveTime <= inactivityThreshold &&
               node.reputation >= 20;
    }
    ...
    function _updateComputePower(NodeCapabilities memory cap, bool add) internal {
        uint256 score = cap.cpuCores * 10 + 
                       cap.ramGB * 5 + 
                       (cap.hasGPU ? cap.gpuMemoryGB * 20 : 0) +
                       cap.bandwidth / 100;
    ...
    function getAvailableMobileNodes() external view returns (address[] memory) {
        ...
        for (uint256 i = 0; i < activeNodes.length; i++) {
            ...
            if (isMobileDevice[node.capabilities.deviceType] && 
                !node.isPowerSaving &&
                node.reputation >= 30) {
                temp[count] = nodeAddr;
                count++;
    ...
    function canNodeHandleTask(address nodeAddress) external view returns (bool) {
        ...
        if (!node.registered || 
            node.activeTasks >= node.deviceProfile.maxConcurrentTasks ||
    ...
    function updateDeviceProfile(
        NodeDeviceType deviceType,
        DeviceProfile calldata newProfile
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
